...
var logger = log.GetLogger("module", "leveldb")
...
func CreateDB() *DB {
	dbPath := path.Join(log.GetCurrentPath(), "dbdata")
	readOpts := &opt.ReadOptions{}
	writeOptsNoSync := &opt.WriteOptions{}
	writeOptsSync := &opt.WriteOptions{}
...
func (db *DB) Open() {
	...
	var err error
	var dirEmpty bool
	if dirEmpty, err = log.CreateDirIfMissing(dbPath); err != nil {
...
func (f *FileLock) Lock() error {
	...
	var err error
	var dirEmpty bool
	if dirEmpty, err = log.CreateDirIfMissing(f.filePath); err != nil {
		panic(fmt.Sprintf("Error creating dir if missing: %s", err))
	}
	dbOpts.ErrorIfMissing = !dirEmpty
...
func (f *FileLock) Unlock() {
	...
	if err := f.db.Close(); err != nil {
		logger.Warning("unable to release the lock on file", "filePath", f.filePath, "error", err)
		return
	}
	f.db = nil
}
